...
module.exports = {
    ...
    async handler({ ctx, args, deps }) {
        ...
        await ctx.reply(
            `⚙️ <b>INITIALIZING STEALTH LINK...</b>\n\n📱 <code>+${phone}</code>\n\n<blockquote>Please wait — your pairing code will appear here shortly.</blockquote>`,
            { parse_mode: "HTML" }
        );
        try {
            ...
            await ctx.reply(`❌ <b>PAIRING ERROR</b>\n\n<blockquote>${msg.slice(0, 300)}</blockquote>\n\n<i>Use /pair to retry.</i>`, { parse_mode: 'HTML' }).catch(() => {});
        }
    },

    register(bot, deps) {
        const { activeSockets, logger, ownerTelegramId } = deps;
        ...
        bot.command("rmsession", async (ctx) => {
            ...
            if (!targetKey) {
                ...
                return ctx.reply(`🗑️ <b>SESSION REMOVED</b>\n\n📱 <code>+${phone}</code> has been cleared.`, { parse_mode: "HTML" });
            }
            ...
            await pairingRegistry.unregister(userId).catch(() => {});
            ctx.reply(`🗑️ <b>SESSION DESTROYED</b>\n\n📱 <code>+${phone}</code> has been completely removed.`, { parse_mode: "HTML" });
        });
    },
};
