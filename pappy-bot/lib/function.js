...
const { proto, areJidsSameUser, extractMessageContent, downloadContentFromMessage, getContentType, getDevice } = require('@crysnovax/baileys');
...
const GIFBufferToVideoBuffer = async (image) => {
    ...
    await fs.writeFileSync(`./PappyMedia/trash/${filename}.gif`, image)
    child_process.exec(
        `ffmpeg -i ./PappyMedia/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./PappyMedia/trash/${filename}.mp4`
    )
    await sleepy(4000)

    var buffer5 = await fs.readFileSync(`./PappyMedia/trash/${filename}.mp4`)
    Promise.all([unlink(`./PappyMedia/trash/${filename}.mp4`), unlink(`./PappyMedia/trash/${filename}.gif`)])
    return buffer5
}
